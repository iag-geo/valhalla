...
insert into testing.valhalla_shape_non_pii
with edge as (
    SELECT trip_id,
           osm_id,
    ...
    WHERE trip_id not in (select trip_id from testing.valhalla_shape_non_pii)
    GROUP BY trip_id,
             osm_id
), mm as (
    select trip_id,
...
from trips
;
analyse testing.valhalla_shape_non_pii;
