...
insert into safer_journeys.osm_waypoint_shape_non_pii
with edge as (
    SELECT trip_id,
           osm_id,
           min(begin_shape_index) as min_point_index,
           max(end_shape_index) as max_point_index
    FROM safer_journeys.osm_waypoint_edge
    WHERE trip_id not in (select trip_id from safer_journeys.osm_waypoint_shape_non_pii)
    GROUP BY trip_id,
             osm_id
), mm as (
    select trip_id,
    ...
    from safer_journeys.osm_waypoint_shape
), trips as (
    select mm.trip_id,
...
from trips
;
analyse safer_journeys.osm_waypoint_shape_non_pii;
